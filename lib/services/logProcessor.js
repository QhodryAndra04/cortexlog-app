...
export function parseApacheLog(line) {
  // Regex yang disesuaikan dengan contoh log Anda
  const regex =
    /^(\S+) \S+ \S+ \[([^\]]+)\] "(\S+)\s+(.*?)\s+(HTTP\/\S+)" (\d+) (\d+|-)(?:\s+"([^"]*)"\s+"([^"]*)")?/;
  const m = line.match(regex);

  if (!m) {
    console.log("Log tidak cocok dengan regex:", line);
    return null;
  }
...
export function buildIpStats(parsedLogs) {
  ...
  for (const log of parsedLogs) {
    ...
    if ([500, 502, 503, 504].includes(log.status_code))
      stats[ip].error_count += 1;
    if (/login|signin|auth/i.test(log.request_url))
      stats[ip].login_attempts += 1;
  }
  return stats;
}
