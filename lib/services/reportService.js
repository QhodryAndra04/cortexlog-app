...
export async function getAllReports(limit = 100, month = null, year = null) {
  let queryStr = `
    SELECT 
      r.id_report,
      r.file_name,
  ...
  if (month && month !== 'all') {
    queryStr += ` AND EXTRACT(MONTH FROM r.generated_at) = $${paramIdx}`;
    params.push(parseInt(month));
    paramIdx++;
  }

  if (year && year !== 'all') {
    queryStr += ` AND EXTRACT(YEAR FROM r.generated_at) = $${paramIdx}`;
    params.push(parseInt(year));
    paramIdx++;
  }

  queryStr += ` ORDER BY r.generated_at DESC LIMIT $${paramIdx}`;
  params.push(limit);
