...
export async function processDetectionBatch(parsedLogs, mlBatchResponse, userId = null) {
  ...
  for (let i = 0; i < parsedLogs.length; i++) {
    ...
    if (result.summary?.is_anomaly) totalAnomalies++;
    if (result.summary?.is_attack) totalAttacks++;
    if (result.summary?.attack_type && result.summary?.attack_type !== 'Normal') {
      attackCounts[result.summary.attack_type] = (attackCounts[result.summary.attack_type] || 0) + 1;
    }
  ...
  if (totalAttacks > 0 || totalAnomalies > 0) {
    let summaryMsg = `📊 *CortexLog Batch Analysis Report*\n\n`;
    summaryMsg += `✅ Total Logs: ${parsedLogs.length}\n`;
    summaryMsg += `🛑 Total Attacks: ${totalAttacks}\n`;
    summaryMsg += `⚠️ Total Anomalies: ${totalAnomalies}\n\n`;
    
    if (Object.keys(attackCounts).length > 0) {
      summaryMsg += `🔍 *Attack Distribution:*\n`;
